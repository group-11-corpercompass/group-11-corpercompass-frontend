...
async function loadConversations() {
  try {
    ...
    messages.forEach(msg => {
      ...
      const otherName = msg.sender._id === currentUserId ? msg.receiver.name : msg.sender.name;
    ...
    list.innerHTML = Array.from(participants.values()).map(p => `
      <li><a href="#" data-id="${p.id}">${escapeHtml(p.name)}</a></li>
    `).join('');
  ...
  try {
    ...
    chatDiv.innerHTML = messages.map(msg => `
      <div class="message ${msg.sender._id === getCurrentUser()._id ? 'sent' : 'received'}">
        <strong>${escapeHtml(msg.sender.name)}:</strong> ${escapeHtml(msg.content)}
      </div>
    `).join('');
